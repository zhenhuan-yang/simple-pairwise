...
def auc_olp_hinge(x_tr, y_tr, x_te, y_te, options):
    ...
    while t < len(ids):
        ...
        if t < buf_size:
            ...
            buf_ids = rs_xx(buf_ids, ids[t], buf_size, t)
        x_t_1 = x_tr[buf_ids]
        y_t_1 = y_tr[buf_ids]
        if y_t > 0:
            idx_neg = y_t_1 < 0
            ...
            idx = ((xx @ w_t) <= 1)
            w_t = w_t + eta_t * np.sum(xx[idx], axis=0) / len(buf_ids)
        else:
            idx_pos = y_t_1 > 0
...
def _auc_olp_hinge(x_tr, y_tr, x_te, y_te, options):
    n_tr,dim = x_tr.shape
    n_pass = options['n_pass']
    n_iter = n_tr * n_pass
    ids = get_idx(n_tr, n_pass)
